        ...
        with urllib.request.urlopen(req) as response:
            with gzip.GzipFile(fileobj=response) as gz:
                with open(out_txt_path, 'wb') as f_out:
                    for line in gz:
                        lines_seen_total += 1
                        if lines_seen_total <= 20_000_000:
                            if lines_seen_total % 500_000 == 0:
                                print(f"\r[{lang.upper()}] Skipping first 20M lines ({lines_seen_total:,}/20,000,000) ...", end="")
                            continue
...
def main():
    ...
    for lang in ['en', 'he']:
        txt_path = os.path.join(data_dir, f'{lang}_corpus_test.txt')
