...
This script compiles the executable with PyInstaller and invokes Inno Setup to create the installer.
"""
...
def create_installer():
    ...
    if not run_command(f'"{sys.executable}" -m PyInstaller SwitchLang.spec --clean', "Building executable with PyInstaller"):
    ...
    # 3. Run Inno Setup compiler passing version definition dynamically
    print(f"--- Creating Installer with Inno Setup (v{__version__}) ---")
    if run_command(f'"{iscc_path}" /DMyAppVersion="{__version__}" setup.iss', "Compiling Setup Script"):
